...
import { pickGoalImage, ImagePermissionDeniedError } from '../../src/utils/imageStorage';
...
export default function AddGoalScreen() {
  ...
  const goal = useGoalsStore((state) => (id ? state.getGoalById(id) : undefined));
  ...
  const handlePickImage = async () => {
    ...
    try {
      ...
      if (error instanceof ImagePermissionDeniedError) {
        ...
        Alert.alert('Gagal memilih gambar', 'Coba lagi beberapa saat lagi.');
      }
    } finally {
      setPickerBusy(false);
    }
  ...
  const handleSave = () => {
    ...
    if (!trimmedName) {
      Alert.alert('Nama goal kosong', 'Kasih nama dulu buat goal tabunganmu.');
      return;
    }
    if (amount <= 0) {
      Alert.alert('Target belum diisi', 'Masukkan target nominal yang mau dicapai.');
      return;
    }

    if (isEditMode && id) {
      updateGoal(id, { name: trimmedName, targetAmount: amount, imageUri, emoji });
    } else {
    ...
    >
      <ScrollView style={styles.container} contentContainerStyle={styles.content}>
        ...
        <View style={styles.imageRow}>
          <Pressable onPress={handlePickImage} disabled={pickerBusy} style={styles.imagePicker}>
            {imageUri ? (
              <Image source={{ uri: imageUri }} style={styles.imagePreview} />
            ) : (
              <Text style={styles.imagePickerText}>{pickerBusy ? '...' : '📷\nUpload'}</Text>
            )}
          </Pressable>
          {imageUri ? (
            <Pressable onPress={() => setImageUri(undefined)} style={styles.removeImageBtn}>
              <Text style={styles.removeImageText}>Hapus gambar, pakai emoji</Text>
            </Pressable>
          ) : null}
        </View>
        ...
        <Pressable onPress={handleSave} style={styles.saveButton}>
          <Text style={styles.saveButtonText}>{isEditMode ? 'Simpan Perubahan' : 'Buat Goal'}</Text>
        </Pressable>
      </ScrollView>
    </KeyboardAvoidingView>
  );
